...
@interface CondorObjectSortFloat()
{
//    void **numbersObject;
//    int size;
    int sizeInsertion;
...
@implementation CondorObjectSortFloat
@synthesize sortFloatProperty;

#pragma mark - OBJECT PROTOCOL 32BIT FLOAT //GOOD
-(NSArray*) sortFloatObjectArray:(NSArray *) array orderDesc: (BOOL) descending selectorNameAsString : (NSString *) sortProperty
{
    ...
    sortFloatProperty = sortProperty;
    selector = NSSelectorFromString(sortFloatProperty);
...
//        float valueFloat = ((float (*)(id, SEL))objc_msgSend)((__bridge id)numbersObject[i],selector);
        unsigned char *stuffBytes = (unsigned char *)numbersObject[i];
        float valueFloat = * ((float *)(stuffBytes + offset));
        
//        IMP f = [(__bridge id)numbersObject[i] methodForSelector:selector];
//        float valueFloat = ((float(*)(void))f)();
//        float valueFloat = (float)([(__bridge id)numbersObject[i] condorIdf]);
        if (valueFloat < 0.0f)
        {
            if (i > low + added[0])
            {
...
//
        ...
        {
            dispatch_async(dispatch_queue_create(nil, DISPATCH_QUEUE_CONCURRENT_WITH_AUTORELEASE_POOL), ^{
                [self sortFloatv2FromLow:added0 toHigh: (high - added1) + 1 withShift: 3 andMaybeSubCount: NULL useGCD:true];
        ...
        if (added1 > 1) {
            dispatch_async(dispatch_queue_create(nil, DISPATCH_QUEUE_CONCURRENT_WITH_AUTORELEASE_POOL), ^{
                [self sortFloatv2FromLow:(high - added1) + 1 toHigh: high + 1 withShift: 3 andMaybeSubCount: NULL useGCD:true];
    ...
    else
    {
        ...
        {
            [self insertionSortFloatObjectWithLow:0 andHigh :size];
        }
    }
}

-(void) sortFloatv2FromLow:(int) low toHigh:(int) high withShift:(int) shift andMaybeSubCount:(int*) subcount useGCD:(BOOL) useGCD
{
    if (high - low > 20)
    {
        int* count;
        ...
        if (subcount == NULL)
        {
            ...
            {
//                UInt32 floatvalue =  [self condorFloatToUInt32: ((float (*)(id, SEL))objc_msgSend)((__bridge id)numbersObject[i],selector)];
//                UInt32 floatvalue = [self condorFloatToUInt32: (float)([(__bridge id)numbersObject[i] condorIdf])];
                unsigned char *stuffBytes = (unsigned char *)numbersObject[i];
                float valueFloat = * ((float *)(stuffBytes + offset));
                UInt32 floatvalue = [self condorFloatToUInt32: valueFloat];
        ...
        while (low < high)
        {
//            UInt32 value =  [self condorFloatToUInt32: ((float (*)(id, SEL))objc_msgSend)((__bridge id)numbersObject[low],selector)];
            //UInt32 value = [self condorFloatToUInt32: (float)([(__bridge id)numbersObject[low] condorIdf])];
            unsigned char *stuffBytes = (unsigned char *)numbersObject[low];
            float valueFloat = * ((float *)(stuffBytes + offset));
            UInt32 value = [self condorFloatToUInt32: valueFloat];
            ...
            if(useGCD) {
                ...
                dispatch_apply(numcpus, queue, ^(size_t i){
                    ...
                    for (long i = start; i < end; i++)
                    {
                        ...
                        {
                            //SORT EACH BOUNDARY
                            [self sortFloatv2FromLow:boundaries[i] toHigh: boundaries[i] + count[i] withShift: shift - 1 andMaybeSubCount: NULL useGCD:false];
                        }
                    }
                });
            } else {
    ...
    {
        [self insertionSortFloatObjectWithLow:low andHigh :high];
    }
}

//Float
-(void) insertionSortFloatObjectWithLow:(int) low andHigh:(int) high
{
    for (int i = low + 1; i < high; i++)
    {
        unsigned char *stuffBytes = (unsigned char *)numbersObject[i];
        float key = * ((float *)(stuffBytes + offset));
        
        void *keyPointer = numbersObject[i];
        int j = i - 1;
        while (j >= low  && (* ((float *)((unsigned char *)numbersObject[j] + offset))) > key)
        {
...
-(void) quickSortObjectWithLow:(int) low andHigh:(int) high
{
    if (high - low > 10)
    {
        ...
        float pivot =  (* ((float *)((unsigned char *)numbersObject[low + (high - low) / 2] + offset)));
        
        while (i < j)
        {
            while ((* ((float *)((unsigned char *)numbersObject[i] + offset))) < pivot)
            {
                i++;
            }
            
            while ((* ((float *)((unsigned char *)numbersObject[j] + offset))) > pivot)
            {
                j--;
            }
    ...
    {
        [self insertionSortFloatObjectWithLow:low andHigh :high + 1];
    }
}


#pragma mark - Helper Methods
-(UInt32) condorFloatToUInt32:(float) f
{
    int val = 0;
